...
for i=1:length(data_clusters)
    if isempty(data_clusters{i})
        error('Error al realizar clustering! \nEl cluster %s está vacio, disminuya la cantidad y pruebe de nuevo', num2str(i))
    end
end
for i=1:c
    for j=1:length(Z(1,:))
